...
class SimpleSwitchStp(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        """
        with open('topo.p', 'rb') as topo: 
            self.rules = pickle.load(topo)
    ...
    def packet_in_handler(self, ev):
        ...
        if hex(_eth_type) == "0x800": 
            ...
            dest_ip = inet_ntoa(dest_ip)

            self.logger.info("SRC IP={} DST IP={}".format(src_ip, dest_ip))

            #enfore the rules
            if dest_ip in self.rules[src_ip]:
                self.logger.info("MATCH FOUND")
            else:
                self.logger.info("NO MATCHING PATH: DROPPING PACKET")
...
#            #Do Protocol based flow management
#            #Protocol Numbers
#            ICMP = 1
#            TCP = 6
#            UDP = 17
...
#                header_len = first & 0b00001111 #IP Pkt header len in number of words
#                
